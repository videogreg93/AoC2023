...
import actions.CameraShakeAction
...
abstract class BasicScreen(
    ...
    internal val batch: SpriteBatch by lazy { MegaManagers.currentContext.inject() }
    internal val hudBatch: SpriteBatch by lazy {  MegaManagers.currentContext.inject() }
    ...
    override fun show() {
        ...
        if (!isAlreadyShown) {
            isAlreadyShown = true
            crew.members.filter { it.shouldStart }.forEach { it.start(crew) }
            firstShown()
        }
    ...
    fun getMembersUnderMouse(list: ArrayList<BaseActor> = crew.members): List<BaseActor> {
        val mousePos = getMousePosition()
        return list.filter {
