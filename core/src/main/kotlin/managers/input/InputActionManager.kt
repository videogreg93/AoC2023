...
import gaia.utils.addAsGestureListener
...
class InputActionManager : KtxInputAdapter, GestureAdapter() {
    ...
    fun init() {
        addAsGestureListener()
        addAsInput()
        val mp = InputMultiplexer(GestureDetector(20f, 0.4f, 0.1f, 999f, this))
        mp.addProcessor(this)
        Gdx.input.inputProcessor = mp
//        controllerManager.init()
    ...
    override fun longPress(x: Float, y: Float): Boolean {
        if (!inputEnabled) return false
        isUsingController = false
        val action = ActionListener.InputAction.LONG_PRESS
        if (disabledInputs.contains(action)) return false
        actionListeners.forEach { listener ->
            if (listener.onAction(action)) return true
        }
...
interface ActionListener {
    ...
    enum class InputAction {
        UP, DOWN, LEFT, RIGHT, ONE, TWO, THREE, FOUR, SEVEN, EIGHT, NINE, ZERO, CLICK, RIGHT_CLICK, SHOOT, START, LONG_PRESS, QUIT
    }
}
